...
//			}
		...
		case 'modif':
		case 'reopen':
			if (!empty($user->hasRight('recurringevent', 'write'))) $object->setDraft($user);
				
			break;
		case 'confirm_validate':
			if (!empty($user->hasRight('recurringevent', 'write'))) $object->setValid($user);
			
			header('Location: '.dol_buildpath('/recurringevent/card.php', 1).'?id='.$object->id);
			exit;

		case 'confirm_delete':
			if (!empty($user->hasRight('recurringevent', 'delete'))) $object->delete($user);
...
else
{
    ...
    else
    {
        ...
        elseif ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
        {
            ...
            /*
            // Ref bis
            $morehtmlref.=$form->editfieldkey("RefBis", 'ref_client', $object->ref_client, $object, $user->hasRight('recurringevent', 'write'), 'string', '', 0, 1);
            $morehtmlref.=$form->editfieldval("RefBis", 'ref_client', $object->ref_client, $object, $user->hasRight('recurringevent', 'write'), 'string', '', null, null, '', 1);
            // Thirdparty
            ...
            */
            ...
            if (empty($reshook))
            {
                ...
                // Modify
                if (!empty($user->hasRight('recurringevent', 'write')))
                {
                ...
                if (!empty($user->hasRight('recurringevent', 'delete')))
                {
