...
class modRecurringEvent extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		//							'workflow' => array('WORKFLOW_MODULE1_YOURACTIONTYPE_MODULE2'=>array('enabled'=>'isModEnabled('module1') && isModEnabled('module2')', 'picto'=>'yourpicto@recurringevent')) // Set here all workflow context managed by module
		//                        );
		$this->module_parts = array(
            'hooks' => array(
                'externalaccesspage'
		...
		// Example: $this->tabs = array('objecttype:+tabname1:Title1:recurringevent@recurringevent:$user->hasRight('recurringevent', 'read'):/recurringevent/mynewtab1.php?id=__ID__',  	// To add a new tab identified by code tabname1
        //                              'objecttype:+tabname2:Title2:recurringevent@recurringevent:$user->hasRight('othermodule', 'read'):/recurringevent/mynewtab2.php?id=__ID__',  	// To add another new tab identified by code tabname2
        ...
        /* Example:
        if (! isModEnabled('recurringevent')) $conf->recurringevent->enabled=0;	// This is to avoid warnings
        $this->dictionaries=array(
            ...
            'tabcond'=>array(isModEnabled('recurringevent'),isModEnabled('recurringevent'),isModEnabled('recurringevent'))												// Condition to show each dictionary
        );
        */
...
*/
		...
		//							'position'=>100,
		//							'enabled'=>'isModEnabled('recurringevent')',	// Define condition to show or hide menu entry. Use 'isModEnabled('recurringevent')' if entry must be visible if module is enabled.
		//							'perms'=>'1',			                // Use 'perms'=>'$user->hasRight('recurringevent', 'level1', 'level2')' if you want your menu with a permission rules
		//							'target'=>'',
		...
		//							'langs'=>'recurringevent@recurringevent',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
		//							'position'=>100,
		//							'enabled'=>'isModEnabled('recurringevent')',  // Define condition to show or hide menu entry. Use 'isModEnabled('recurringevent')' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
		//							'perms'=>'1',			                // Use 'perms'=>'$user->hasRight('recurringevent', 'level1', 'level2')' if you want your menu with a permission rules
		//							'target'=>'',
		//							'user'=>2);				                // 0=Menu for internal users, 1=external users, 2=both
		// $r++;
...
/*
		$this->menu[$r]=array(
			...
			'langs'=>'recurringevent@recurringevent',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>100+$r,
			'enabled'=>'isModEnabled('recurringevent')',	// Define condition to show or hide menu entry. Use 'isModEnabled('missionorder')' if entry must be visible if module is enabled.
			'perms'=>'$user->hasRight('recurringevent', 'read')',			                // Use 'perms'=>'$user->hasRight('missionorder', 'level1', 'level2')' if you want your menu with a permission rules
			'target'=>'',
			'user'=>0
		);
		$r++;
		...
		$this->menu[$r]=array(
			...
			'langs'=>'recurringevent@recurringevent',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>100+$r,
			'enabled'=> 'isModEnabled('recurringevent')',  // Define condition to show or hide menu entry. Use 'isModEnabled('missionorder')' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			'perms'=> '$user->hasRight('recurringevent', 'write')',			                // Use 'perms'=>'$user->hasRight('missionorder', 'level1', 'level2')' if you want your menu with a permission rules
			'target'=>'',
			'user'=>0
